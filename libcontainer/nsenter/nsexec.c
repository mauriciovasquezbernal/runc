...
enum sync_t {
	...
	SYNC_MOUNTSOURCES_PLS = 0x42,	/* Tell parent to send mount sources by SCM_RIGHTS. */
	SYNC_MOUNTSOURCES_ACK = 0x43,	/* All mount sources have been sent. */
	SYNC_RECVPID_PLS = 0x44,	/* Tell parent we're sending the PID. */
	SYNC_RECVPID_ACK = 0x45,	/* PID was correctly received by parent. */
	SYNC_GRANDCHILD = 0x46,	/* The grandchild is ready to run. */
	SYNC_CHILD_READY = 0x47,	/* The child or grandchild is ready to return. */
};
...
void receive_mountsources(int sockfd, char *mountsources, size_t mountsources_len)
{
	...
	// container_linux.go sendMountSources() decides if mount sources
